...
def index_donor(request):
        ...
        acceptedcount = Donation.objects.filter(donor=donor, status="accpet").count()
        rejectedcount = Donation.objects.filter(donor=donor, status="reject").count()
        pendingcount = Donation.objects.filter(donor=donor, status="panding").count()
            ...
            if form.is_valid():
                ...
                try:
                    Donation.objects.create(donor=donor, donationname=donationname, donationpic=donationpic,
                                            ...
                                            description=description, status="pending", donationdate=date.today())
...
def index_volunteer(request):
        ...
        totalNotRecDonation = Donation.objects.filter(volunteer=volunteer,
                                                        status="Donation delivered successfully").count()
        totalDonationDelivered = Donation.objects.filter(volunteer=volunteer,
                                                            status="Donation Delivered Successfully").count()
...
class changepwd_volunteer(View):
        ...
        def post(self, request):
            ...
            if not request.user.is_authenticated:
                return redirect('/login-volunteer')
            old = request.POST['old_password']
            newpass = request.POST['old_password']
            confirmpass = request.POST['new_password2']
            try:
                if newpass == confirmpass:
                    user = User.objects.get(id=request.user.id)
                    if user.check_password(old):
                        user.set_password(newpass)
                        user.save()
                        messages.success(request, 'Change Password Succcessfully')
                    else:
                        messages.warning(request, 'Old password not method')

                else:
                    messages.warning(request, 'Old Password and New Password are different')
            except:
                messages.warning(request, 'Filed to change password')
...
class donationcollection_detail(View):
        ...
        def post(self, request, pid):
            ...
            try:
                ...
                messages.warning(request, 'Failed to updated volunteer status and remark')
